user_link = input("Copy and paste the link here: ")
link = ["https://www.youtube.com/watch?v=xvFZjo5PgG0","https://www.youtube.com/watch?v=xvFZjo5PgG0","https://www.youtube.com/watch?v=G8iEMVr7GFg"]

print("working...")

if user_link == link:
    print("This is most likely a rickroll link")

if user_link != link:
    print("This is a not a rickroll most likely.")

print("Our database is currently getting bigger with more possible rickroll links.")